...
describe("EditarClase", () => {
  ...
  beforeEach(() => {
    ...
    localStorage.setItem("role", "tutor"); // si usas este en el componente para redirección
  });
  ...
  it("permite editar y enviar el formulario", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/clase actualizada con éxito/i)
      ).toBeInTheDocument()
    );
  });
  ...
  it("muestra error si la petición de actualización falla", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/error al actualizar clase/i)
      ).toBeInTheDocument();
    });
  });
});
